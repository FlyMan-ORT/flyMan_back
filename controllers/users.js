...
const register = async (req, res) => {
    try {
        ...
        if (!name || !email || !password || !phone) return res.status(400).json({error: "No se pueden enviar campos vacíos."});
...
const updateUser = async (req, res) => {
    try {
        ...
        if (!id || !name || !email || !phone) return res.status(400).json({error: "No se pueden enviar campos vacíos."});
